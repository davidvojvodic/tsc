...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`http://${CAMERA_HOST}:${CAMERA_PORT}${STREAM_ENDPOINT}`, {
      headers: {
        'Authorization': `Basic ${auth}`,
        'Accept': '*/*',
      },
      // @ts-expect-error - Edge runtime supports streaming
      signal: request.signal, // Pass through the request signal for cancellation
    });
    ...
    const contentType = response.headers.get('content-type') || 'multipart/x-mixed-replace; boundary=myboundary';
